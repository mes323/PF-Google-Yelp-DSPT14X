...
LOCAL_FOLDER = '/home/airflow/gcs/data/yelp_bulk'  # Montado desde D:\tmp\yelp_bulk
GCS_TARGET_FOLDER = 'raw/yelp/'
CLOUD_RUN_URL = 'https://review-gmaps-808937578837.us-central1.run.app'
BQ_OUTPUT_TABLE = 'datawave-proyecto-final.yelpdataset.data'
...
def enviar_telegram_mensaje():
    mensaje = "✅ DAG Finalizado: dag_yelp_upload_process"
...
with DAG(
    dag_id='dag_yelp_upload_process',
    default_args=default_args,
    schedule_interval=None,
    catchup=False,
    tags=['yelp', 'cloud_run', 'bigquery']
) as dag:

    start = EmptyOperator(task_id='start')

    upload_task = PythonOperator(
        task_id='upload_yelp_to_gcs',
        python_callable=upload_files_to_gcs
    )

    process_task = PythonOperator(
        task_id='process_yelp_with_cloud_run',
